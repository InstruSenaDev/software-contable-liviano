...
const $$RegistroP = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${maybeRenderHead()}<body class="bg-gray-100 p-8"> <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md"> <span class="flex flex-row "> <div class="mt-4 p-2"> <button type="button" id="volver" class="bg-blue-baby4  text-white p-2 rounded hover:bg-blue-baby3" onclick="window.history.back()">Volver</button> </div> </span> <h1 class="text-3xl font-display text-blue-baby4 text-center font-bold mb-6 p-5">
